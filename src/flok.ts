...
function compile(dir: FlockDirectory) {
  return minify(
    Language.JS,
    `void ((()=>{window.swarm = ${compileAllCode(dir)}})())`,
  );
}
...
class WebBuilderBuilder {
  ...
  generateGenerator(
    ...
    const compileDir = (dir: FlockDirectory, requirement: string) => {
      ...
      output += `o += "(()=>{";`;
      output += `o += ${
        JSON.stringify(`const b = (()=>{${minify(Language.JS, dir.src)}})();`)
      };`;
      for (const sub in dir.children) {
        output += `if (${
          this.dependencyResolver.modules[requirement + "." + sub].dependents
            .map((dependant) =>
              `${dependenciesName}.includes(${JSON.stringify(dependant)})`
            ).join(" || ")
        }) {`;
        output += `o += ${
          "`" +
          `b.${sub} = \${${
            compileDir(dir.children[sub], requirement + "." + sub)
          }};` + "`"
        };`;
        output += `}`;
      }
    ...
    let compiled = JSON.stringify("void ((()=>{window.swarm = ") + " + " +
      compileDir(dir, "swarm") + " + " + JSON.stringify("})())");
